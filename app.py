...
ADMIN_ACCESS_TOKEN = "TEACHER_SECURE_2026"  # Admin Secret Code
...
def chat():
    ...
    # --- CRITICAL FIX: Validation Checks ---
    if not subject: 
        return jsonify({"error": "Please select a subject."}), 400
        
    if not user_query: # <--- Prevents TypeError: 'NoneType' object is not iterable
    ...
    try:
        ...
        # Build Context String with Page Numbers
        context_text = ""
...
def generate_quiz():
    ...
    try:
        ...
        if not context_text: return jsonify({"error": "Not enough content."}), 400
